...
impl<'a> Element<'a> {
    ...
    pub fn from_str(xml: &'a str) -> Result<Self, DomError> {
        Self::from_reader(Reader::from_str(xml))
    }

    fn from_reader(mut reader: Reader<&'a [u8]>) -> Result<Self, DomError> {
        ...
        loop {
            let (unprocessed, create_text_node) = match reader.read_event()? {
                ...
                Event::CData(e) => (Unprocessed::Text(e.decode()?), false),
            ...
            match unprocessed {
                ...
                Unprocessed::End(_) => match parents.pop() {
...
mod tests {
    ...
    fn missing_open_tag() {
        ...
        assert!(matches!(
            error,
            DomError::Parse(Error::EndEventMismatch { .. })
        ));
    }
}
