...
use crate::utils::{CowRef, CowStrVisitor};
use serde::de::value::BorrowedStrDeserializer;
use serde::de::{
    Deserialize, DeserializeSeed, Deserializer, Error, IntoDeserializer, MapAccess, Visitor,
};
...
impl<'de> MapAccess<'de> for ElementMapAccess<'de> {
    ...
    fn next_key_seed<K: DeserializeSeed<'de>>(
        ...
        match self.children.next() {
            ...
            /*Some(Node::Element(e)) if self.has_value_field && not_in(self.fields, e, self.decoder)? => {
                let de = BorrowedStrDeserializer::<DeError>::new(VALUE_KEY);
                seed.deserialize(de).map(Some)
            }*/
            Some(Node::Element(e)) => {
    ...
    macro_rules! same {
        ($name:ident, $xml:literal) => {
            ...
            fn $name() {
                ...
                // tags is not equal because MapAccess does not provide the struct name
