...
impl<'de> ElementMapAccess<'de> {
    ...
    fn not_in(&self, element: &Element<'de>) -> Result<bool, EncodingError> {
        let tag = element.tag.local_name().into_inner();

        Ok(self.fields.iter().all(|&field| field != tag))
    }
}
...
impl<'de> MapAccess<'de> for ElementMapAccess<'de> {
    ...
    fn next_key_seed<K: DeserializeSeed<'de>>(
        ...
        if let Some(a) = self.iter.next(self.attributes.as_bytes()) {
            ...
            let de = QNameDeserializer::from_attr(key, &mut self.key_buf)?;
    ...
    fn next_value_seed<K: DeserializeSeed<'de>>(
        ...
        match std::mem::replace(&mut self.pending_value, Value::Unknown) {
            Value::Attribute(value) => seed.deserialize(SimpleTypeDeserializer::from_attr(
                &self.attributes,
                value,
                crate::XmlVersion::Implicit1_0,
            )),
