...
impl<'i> Element<'i> {
    ...
    pub fn from_str(xml: &'i str) -> Result<Self, DomError> {
        ...
        loop {
            match builder.feed(event, version)? {
                FeedResult::NeedData => {
                    event = reader.read_event()?;
                    continue;
                }
    ...
    pub fn from_reader<R: BufRead>(reader: R) -> Result<Element<'static>, DomError> {
        ...
        let event = match reader.read_event_into(&mut buf)?.into_owned() {
            Event::Decl(e) => {
                version = e.xml_version()?;
                reader.read_event_into(&mut buf)?.into_owned()
            }
            e => e,
        };
        // Skip comments, processing instructions and spaces
...
impl<'i> DomBuilder<'i> {
    ...
    pub fn feed(
        ...
        event: Event<'i>,
        version: XmlVersion,
    ) -> Result<FeedResult<'i>, EncodingError> {
        ...
        match unprocessed {
            ...
            Unprocessed::Text(e) => self.append_text(e.xml_content(version)),
            Unprocessed::CData(e) => self.append_text(e.xml_content(version)),
        }
